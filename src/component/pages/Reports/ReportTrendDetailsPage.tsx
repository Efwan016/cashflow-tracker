      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 sm:mb-10 rounded-3xl sm:rounded-[40px] border border-black/5 dark:border-white/10 bg-white dark:bg-slate-950/80 p-5 sm:p-8 shadow-xl dark:shadow-[0_30px_120px_-50px_rgba(15,23,42,0.85)] backdrop-blur-xl transition-colors">
          <div className="flex items-start sm:items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.35em] text-sky-600 dark:text-sky-300/80">Trend Analysis</p>
              <h1 className="mt-2 text-xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Cashflow & Trend Insights</h1>
            </div>
          </div>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Deep dive into revenue, profit, and expense trends with detailed visual analytics.
          </p>
        </div>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-sm text-slate-500 animate-pulse">Calculating trends and insights...</p>
          </div>
        )}
        ...
        {!loading && !error && trendChartData.revenue.length === 0 && trendChartData.expense.length === 0 && (
          <div className="mb-10 rounded-[40px] border border-slate-200 bg-white p-8 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950/80 dark:text-slate-400">
            <p className="font-bold mb-2">No Data Available</p>
            <p>There are no transactions or expenses recorded for the selected period. Please adjust your date range.</p>
          </div>
        )}
        ...
        <section className="mb-10 rounded-3xl sm:rounded-[40px] border border-slate-200 bg-white p-5 sm:p-8 shadow-xl dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Daily Cashflow Overview</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Revenue, expense, and profit trend</p>
            </div>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
            Track daily movement across your selected period ({dateRangeLabel})
          </p>
        ...
        <section className="mb-10 rounded-3xl sm:rounded-[40px] border border-slate-200 bg-white p-5 sm:p-8 shadow-xl dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Average Revenue by Day</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Discover which days of the week perform best on average</p>
            </div>
          </div>
        ...
        <section className="mb-10 grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2">
          <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/5 p-6 sm:p-8 dark:bg-emerald-500/10">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold">Peak Revenue Day</h3>
                <p className="text-sm text-slate-500">Your highest performing date</p>
              </div>
            </div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{fmt.format(peakRevenue)}</div>
            <p className="mt-2 text-sm font-medium text-emerald-600 uppercase tracking-tighter italic">Achieved on {peakDate}</p>
          </div>
          ...
          <div className="rounded-3xl border border-rose-500/20 bg-rose-500/5 p-6 sm:p-8 dark:bg-rose-500/10">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold">Highest Spending Day</h3>
                <p className="text-sm text-slate-500">Day with maximum operational cost</p>
              </div>
            </div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{fmt.format(peakExpense)}</div>
            <p className="mt-2 text-sm font-medium text-rose-600 uppercase tracking-tighter italic">Occurred on {peakExpenseDate}</p>
          </div>
        </section>
        ...
        <section className="mb-10 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="rounded-3xl sm:rounded-[40px] border border-slate-200 bg-white p-6 sm:p-8 shadow-xl dark:border-slate-700 dark:bg-slate-950/80">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Momentum</h3>
        ...
        <section className="mb-10 rounded-3xl sm:rounded-[40px] border border-slate-200 bg-white p-6 sm:p-8 shadow-xl dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Month Comparison</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">This month vs last month</p>
            </div>
          </div>
        ...
        <section className="rounded-[32px] border border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100 p-8 dark:border-slate-700 dark:from-slate-900/50 dark:to-slate-900/30">
          ...
          <div className="grid gap-6 md:grid-cols-3 text-sm">
            <div>
              <p className="font-medium text-slate-900 dark:text-white mb-2">Trend Direction</p>
              <p className="text-slate-600 dark:text-slate-300">
                Monitor the overall direction of your revenue and profit over the selected period.
              </p>
            </div>
            <div>
              <p className="font-medium text-slate-900 dark:text-white mb-2">Monthly Performance</p>
              <p className="text-slate-600 dark:text-slate-300">
                Compare this month's performance with the previous month to identify growth patterns.
              </p>
            </div>
            <div>
              <p className="font-medium text-slate-900 dark:text-white mb-2">Budget Planning</p>
              <p className="text-slate-600 dark:text-slate-300">
                Use these insights to plan your budget and set realistic growth targets.
              </p>
            </div>
          </div>
        </section>
