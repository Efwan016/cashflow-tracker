    ...
    <>
      <section className="mt-8 grid gap-6 xl:grid-cols-3">
        <Link to={`/reports/insights/best-by-quantity?type=${filterType}&start=${startDate}&end=${endDate}`} className="group rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg hover:-translate-y-1 dark:border-slate-700 dark:bg-slate-950/80">
          <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Product insights')}</p>
          <h3 className="mt-3 text-xl font-semibold text-slate-900 dark:text-white">{t('Best seller by quantity')}</h3>
          <div className="mt-6 space-y-4">
              {bestByQty.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t('No product sales available yet.')}</div>
            ) : (
              bestByQty.map((item, index) => (
                <div key={item.name} className="space-y-2 rounded-3xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/90">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span>{t('Qty')} {num.format(item.qty)}</span>
                    <span>{t('Revenue')} {fmt.format(item.revenue)}</span>
                  </div>
        ...
        <Link to={`/reports/insights/best-by-revenue?type=${filterType}&start=${startDate}&end=${endDate}`} className="group rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg hover:-translate-y-1 dark:border-slate-700 dark:bg-slate-950/80">
          <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Product insights')}</p>
          <h3 className="mt-3 text-xl font-semibold text-slate-900 dark:text-white">{t('Best seller by revenue')}</h3>
          <div className="mt-6 space-y-4">
            {bestByRevenue.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t('No revenue data available.')}</div>
            ) : (
              bestByRevenue.map((item, index) => (
                <div key={item.name} className="space-y-2 rounded-3xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/90">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-3 text-sm text-slate-600 dark:text-slate-300">
                    <span>{t('Revenue')} {fmt.format(item.revenue)}</span>
                    <span>{t('Profit')} {fmt.format(item.profit)}</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
        ...
        <Link to={`/reports/insights/most-profitable?type=${filterType}&start=${startDate}&end=${endDate}`} className="group rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg hover:-translate-y-1 dark:border-slate-700 dark:bg-slate-950/80">
          <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Product insights')}</p>
          <h3 className="mt-3 text-xl font-semibold text-slate-900 dark:text-white">{t('Most profitable product')}</h3>
          <div className="mt-6 space-y-4">
            {mostProfitable.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t('Profit data will appear once sales are recorded.')}</div>
            ) : (
      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-950/80">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Expense insights')}</p>
                <h3 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">{t('Top categories')}</h3>
              </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t('Summarized by description.')}</p>
          </div>
          <div className="mt-6 space-y-4">
            {expenseBreakdown.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t('No expense breakdown found.')}</div>
            ) : (
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-950/80">
            <div>
            <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Expense insights')}</p>
            <h3 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">{t('Biggest expense')}</h3>
          </div>
          {biggestExpense ? (
            <div className="mt-6 space-y-4 rounded-[28px] border border-slate-100 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-900/90">
              <div className="flex items-center justify-between gap-4">
                <p className="text-lg font-semibold text-slate-900 dark:text-white">{biggestExpense.description || t('Expense')}</p>
            ...
            <div className="mt-6 rounded-3xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">{t('No expenses were found in this period.')}</div>
          )}
        </div>
      </section>
      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[1fr_0.95fr]">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Inventory insights')}</p>
              <h3 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">{t('Stock value and opportunity')}</h3>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t('Based on current inventory and pricing.')}</p>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/90">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('Stock value')}</p>
              <p className="mt-3 text-2xl font-semibold text-slate-900 dark:text-white">{fmt.format(stockSummary.value)}</p>
            </div>
            <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/90">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('Potential revenue')}</p>
              <p className="mt-3 text-2xl font-semibold text-slate-900 dark:text-white">{fmt.format(stockSummary.potentialRevenue)}</p>
            </div>
            <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/90">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('Potential profit')}</p>
              <p className="mt-3 text-2xl font-semibold text-slate-900 dark:text-white">{fmt.format(stockSummary.potentialProfit)}</p>
            </div>
            <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/90">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('Low stock alert')}</p>
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-950/80">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('Inventory alerts')}</p>
              <h3 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">{t('Low stock products')}</h3>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t('Restock priority.')}</p>
          </div>
          <div className="mt-2 space-y-1">
            {stockSummary.lowStock.length === 0 ? (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-200">
                {t('All stock levels look healthy.')}
              </div>
            ) : (
              pagedLowStock.map((item) => (
                <div key={`${item.name}-${item.qty}`} className="rounded-xl border border-slate-100 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-900/90">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:bg-rose-500/10 dark:text-rose-200">{num.format(item.qty)} {t('left')}</span>
                  </div>
                  <p className="mt-0.5 text-[10px] text-slate-500 dark:text-slate-400 leading-tight">{t('Cost')} {fmt.format(item.modal)} · {t('Retail')} {fmt.format(item.jual)}</p>
                </div>
              ))
            )}
          </div>
